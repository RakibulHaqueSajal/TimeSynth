...
Input CSV format is the output of Statistical_Test/noise.py:
    signal,metric,level,model,n_paired,delta_vs_linear,ci_lo,ci_hi,t_approx,p_value,p_holm

Default outputs:
  - line plot: delta vs Linear across noise levels
  - heatmap:   delta vs Linear by model x level
...
DEFAULT_CSV_DIR = (
    ...
    "Time_Series_Forecast/Model_Comparison/Statistical/Noise_Dir"
)

# Auto-discover all OptionA CSVs
DEFAULT_CSVS = sorted([
    os.path.join(DEFAULT_CSV_DIR, f)
    for f in os.listdir(DEFAULT_CSV_DIR)
    if f.startswith("OptionA_") and f.endswith("_per_level.csv")
...
def build_model_series(
    ...
    for row in rows:
        model = str(row["model"])
        idx = level_to_idx[int(row["level"])]
...
def plot_line_comparison(
    ...
    ax.set_title(f"{signal}: model comparison across noise levels ({metric})")
...
def plot_heatmap(
    ...
    # --- Priority 1 #4: Sort models by average performance (best first) ---
    ...
    # (no summary averages — keep figure clean)
    ...
    # --- Priority 8: Gridlines between cells ------------------------------
    ...
    ax.set_xticklabels(
        [f"SNR {lvl}" for lvl in levels],
        fontsize=13, fontweight="bold",
    )
    ax.set_xlabel(
        "Noise Level (SNR: Low \u2192 High)",
    ...
    # --- Priority 1 #3: Better title + subtitle ---------------------------
    sig_label = signal.replace("_", " ")
    metric_nice = {"mae": "\u0394MAE", "freq": "\u0394|Freq Error|", "phase": "\u0394|Phase Error|"}.get(metric, metric)
    ax.set_title(
        f"Model Performance Across Noise Levels \u2014 {metric_nice}\n"
        f"({sig_label})",
        fontweight="bold", fontsize=17, pad=14,
    )
    # Subtitle explaining the sign convention
    # ax.text(
    #     0.5, 1.01,
    #     "Positive (blue) = improvement over Linear baseline; "
    #     "Negative (red) = worse than baseline",
    #     transform=ax.transAxes, ha="center", va="bottom",
    #     fontsize=8.5, fontstyle="italic", color="#444444",
    # )

    # --- Priority 1 #2 & #5: Value labels + significance in cells ---------
    for i, model in enumerate(plot_models):
        ...
        for j in range(n_levels):
            ...
            if not np.isfinite(value):
                ...
                # Normalize value to [0, 1] range for luminance check
    ...
    # --- Priority 7: Enhanced color bar with direction labels -------------
    ...
    # --- Significance note (compact, below colorbar) ----------------------
    ax.text(
        1.0, -0.06,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot model comparison figures for Option A noise results.")
    parser.add_argument(
        "--csv", nargs="*", default=None,
        help="Path(s) to Option A CSV file(s). Defaults to all OptionA CSVs found.",
    )
