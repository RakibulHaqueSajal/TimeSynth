...
from Layers.Pathformer_Layer import Transformer_Layer
...
class AMS(nn.Module):
    ...
    def _prob_in_top_k(self, clean_values, noisy_values, noise_stddev, noisy_top_values):
        ...
        val_in = (clean_values - threshold_if_in) / noise_stddev
        val_in = torch.nan_to_num(val_in, nan=0.0, posinf=10.0, neginf=-10.0)
        prob_if_in = normal.cdf(val_in)
        val_out = (clean_values - threshold_if_in) / noise_stddev
        val_out = torch.nan_to_num(val_out, nan=0.0, posinf=10.0, neginf=-10.0)
        prob_if_out = normal.cdf(val_out)
