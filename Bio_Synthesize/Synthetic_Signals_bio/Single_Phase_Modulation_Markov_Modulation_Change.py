...
# ======================================================
#  TWO-STATE MARKOV SWITCHING VERSION
#  A, f, β, fmod ALL CHANGE WITH STATE
# ======================================================
...
# ----- Better-separated frequency bands (carrier f) -----
...
# ----- Better-separated modulation frequency bands (fmod) -----
def make_well_separated_fmod_ranges(global_fmod_range):
    """
    Given (fmod_low, fmod_high), create two distinct bands for modulation
    frequency in state 0 and state 1.
    """
    fmod_low, fmod_high = global_fmod_range
    span = fmod_high - fmod_low

    # State 0: slower modulation
    fmod0_range = (fmod_low + 0.05 * span,
                   fmod_low + 0.30 * span)

    # State 1: faster modulation
    fmod1_range = (fmod_low + 0.55 * span,
                   fmod_low + 0.90 * span)

    return fmod0_range, fmod1_range

# ----- Hash for 2-state parameters (now includes A0, A1) -----
def param_hash_pm_two_state(
    A0, A1, f0, f1, beta0, beta1, fmod0, fmod1, offset, p, precision=6
):
    key = (
        f"{A0:.{precision}f}_"
        f"{A1:.{precision}f}_"
        f"{f0:.{precision}f}_"
        f"{f1:.{precision}f}_"
        f"{beta0:.{precision}f}_"
        f"{beta1:.{precision}f}_"
        f"{fmod0:.{precision}f}_"
        f"{fmod1:.{precision}f}_"
        f"{offset:.{precision}f}_"
        f"{p:.{precision}e}"
    )
    return hashlib.md5(key.encode()).hexdigest()

# ----- Two-state phase-modulated signal (A, f, β, fmod per state) -----
def generate_two_state_pm_signal(
    ...
    """
    Two-state PM signal where amplitude, carrier frequency, modulation depth,
    and modulation frequency all change with state.
    ...
    Modulation:
        A_t     = A0 or A1
        beta_t  = beta0 or beta1
        fmod_t  = fmod0 or fmod1
        phase_mod_k = beta_t * sin(2π fmod_t * t_k)

    Signal:
        x_k = A_t * sin(θ_k + phase_mod_k) + offset
    """
    dt = 1.0 / fs
    t = np.arange(T) * dt
...
def save_two_state_pm_signal(
    ...
    filename = (
        ...
        f"offset_{offset:.4f}_p_{p:.5e}.csv"
    )
    df = pd.DataFrame(
        {
            "Time": t,
...
def generate_two_state_pm_dataset(
    base_dir,
    param_bounds,
    transition_probs=(0.0, 0.1, 0.5, 0.9, 1.0),
    n_train=70,
    n_val=10,
    n_test=20,
    fs=10,
    duration=300,
    # optional: custom ranges for the two states
    f0_range=None,
    f1_range=None,
    fmod0_range=None,
    fmod1_range=None,
    ...
    """
    Generate PM signals with 2-state switching in:
        - amplitude A
        - carrier frequency f
        - modulation depth β
    ...
    # Use better-separated carrier frequency bands if not provided
    if f0_range is None or f1_range is None:
        f0_range, f1_range = make_well_separated_freq_ranges(param_bounds["f"])

    # Use better-separated modulation frequency bands if not provided
    ...
    for p in transition_probs:
        ...
        for mode, count in splits.items():
            ...
            while idx_local < count:
                # Offset shared across states
                offset = np.random.uniform(*param_bounds["offset"])

                # Amplitude: base + small shift for state 1
                A_base = np.random.uniform(*param_bounds["A"])
                delta_A = np.random.uniform(0.01, 0.03)  # subtle but noticeable
                A0 = A_base
                A1 = A_base + delta_A
                ...
                # Modulation depth: base + small shift for state 1
                beta_base = np.random.uniform(*param_bounds["beta"])
                delta_beta = np.random.uniform(0.02, 0.04)
                beta0 = beta_base
                beta1 = beta_base + delta_beta

                # Modulation frequencies
                fmod0 = np.random.uniform(*fmod0_range)
                fmod1 = np.random.uniform(*fmod1_range)

                h = param_hash_pm_two_state(
                    A0, A1, f0, f1, beta0, beta1, fmod0, fmod1, offset, p
                )
                if h in used_hashes:
                    continue
                used_hashes.add(h)
...
def visualize_signals(base_dir, split="train", num_samples=3, max_rows=50):
    """
    Simple visualizer for CSV files with columns: Time, Value.
    For two-state data, point this to one specific p-folder:
        base_dir/p_<p>/ and then split="train"/"val"/"test".
    """
...
if __name__ == "__main__":
    ...
    two_state_base_dir = (
        "/scratch_nvme/Time_Series/Bio-Synthesize/"
        "Generation_Synthesized_Bio_Signals/PhaseMod_Single_Freq_TwoState_Modulation_Change_Extended"
    )
    ...
    # Example quick look for a specific p:
